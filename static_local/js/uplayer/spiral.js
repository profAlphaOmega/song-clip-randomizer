...
function init() {
    ...
    renderer.domElement.onclick = function() { $('#modal').iziModal('toggle'); };
...
function animateParticles(){
    ...
    for (var j = 0; j <= particles.length; j++){
        ...
        if (spiral.toggleRed){
            ...
            var R = Math.random() * spiral.R + (timeFloatData[j]);
            var G = Math.random() * spiral.G - (timeFloatData[j]);
            var B = Math.random() * spiral.B - (timeFloatData[j]);
